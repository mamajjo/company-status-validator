	...
	return fetch(params, {
		...
		.then(json => {
			if (JSON.stringify(json) == '{"code":"HTTP-503","message":"Usługa niedostępna"}') {
				...
				logger.log({
					level: 'info',
					message: json.result.subjects[0].name + " " + json.result.subjects[0].nip + " " + json.result.subjects[0].accountNumbers.filter(number => number == ba.toString())[0] + " " + json.result.subjects[0].statusVat.toString(),
				})
				return {
					name: json.result.subjects[0].name,
					nip: json.result.subjects[0].accountNumbers.filter(number => number == ba.toString())[0],
		...
		return fetch(params, {
			...
			.then(json => {
				if (
					...
					return {
						name: json.result.subject.name,
						idNumber: json.result.subject.nip,
						statusVat: json.result.subject.statusVat,
						message: json.result.subject.message || ""
					}
				}
	...
	getStatusByBA: ba => {
		...
		.then(json => {
			if (JSON.stringify(json) == '{"code":"HTTP-503","message":"Usługa niedostępna"}') {
				...
				logger.log({
					level: 'info',
					message: json.result.subjects[0].name + " " + json.result.subjects[0].nip + " " + json.result.subjects[0].accountNumbers.filter(number => number == ba)[0] + " " + json.result.subjects[0].statusVat.toString(),
				})
				return {
					name: json.result.subjects[0].name,
					idNumber: json.result.subjects[0].accountNumbers.filter(number => number == ba)[0],
					statusVat: json.result.subjects[0].statusVat,
					message: json.result.subjects[0].message || ""
				}
			}
