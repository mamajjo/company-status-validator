		...
		.then(json => {
			if (JSON.stringify(json) == '{"code":"HTTP-503","message":"Usługa niedostępna"}') {
				...
				return {
					...
					id: json.result.subjects[0].accountNumbers.filter(number => number == ba.toString())[0],
	...
	getStatusByBA: ba => {
		...
		.then(json => {
			if (JSON.stringify(json) == '{"code":"HTTP-503","message":"Usługa niedostępna"}') {
				...
				return {
					name: json.result.subjects[0].name,
					id: json.result.subjects[0].accountNumbers.filter(number => number == ba)[0],
					statusVat: json.result.subjects[0].statusVat,
					message: json.result.subjects[0].message || ""
				}
			}
