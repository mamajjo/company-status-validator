...
module.exports = {
    checkGivenArguments: (args) => {
        ...
        let correctedNIPs = [];
        let correctedBAs = [];
        let noValidationPassNumbers = [];
        let wrongNumbers = [];
        argsWithoutVariable.forEach(number => {
            number = number.replace(/-/g, '');
            number = number.replace(/\s/g, '');
            if (number.length == 10 ) {
                if (!nipValidation(number)) {
                    arbiter = false;
                    noValidationPassNumbers.push(number);
                    return;
                }
                correctedNIPs.push(number);
            } else if (number.length == 26) {
                if (!bankAccountNumberValidation(number)) {
                    arbiter = false;
                    noValidationPassNumbers.push(number);
                    return;
                }
        ...
        return {
            NIPs: correctedNIPs,
            BAs: correctedBAs,
            NoValidationPass: noValidationPassNumbers,
